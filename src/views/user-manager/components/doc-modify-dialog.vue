<template>
  <el-dialog title="修改"
    ...
    <el-form :model="ruleForm"
      ...
      <div class="a-form-item">
        <label for="slug">控制授权：</label>
        <el-form-item prop="slug">
          <el-radio-group v-model="ruleForm.slug"
                          size="small">
            <el-radio :label="1"
                      border>是</el-radio>
            <el-radio :label="0"
...
export default {
  ...
  data () {
    ...
    const validateRequired1 = (rule, value, callback) => {
      if (!value) {
        callback()
      } else if (value.length < 8 || value.length > 40) {
        callback(new Error('请确认密码长度在8～40之间'))
      } else {
        callback()
      }
    }
    ...
    return {
      ...
      loginRules: {
        ...
        password: [{ required: true, validator: validateRequired1 }],
        state: [{ required: true, validator: validateRequired }],
        slug: [{ required: true, validator: validateRequired }],
