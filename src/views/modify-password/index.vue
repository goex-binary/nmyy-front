<template>
  <div class="register-container">
    ...
    <p class="register-title">
      <span @click="toBack"
            class="el-page-header__left back-btn"><i class="el-icon-back"></i><span class="el-page-header__title">返回</span></span>
      修改密码
    </p>
      ...
      <el-form ref="registerForm"
        ...
        <div class="r-f-footer">
          <el-button :loading="loading"
                     type="primary"
                     class="submit-btn"
                     @click.native.prevent="handleregister">提交</el-button>
        </div>
      </el-form>
    </div>
    <div v-else
...
export default {
  ...
  data () {
    ...
    return {
      ...
      registerRules: {
        ...
        oldPassword: [{ required: true, trigger: 'blur', validator: validateComfirmPassword }]
      },
      loading: false,
  ...
  computed: {
    strestrength () {
      let pwd = this.registerForm.newPassword ? _.trim(this.registerForm.newPassword) : ''
      let className = ''
      if (!pwd || pwd.length < 8) {
        return ''
      }
  ...
  methods: {
    ...
    handleregister () {
      this.$refs.registerForm.validate(valid => {
        if (valid) {
          this.loading = true
          modifyPassword(this.registerForm).then(() => {
