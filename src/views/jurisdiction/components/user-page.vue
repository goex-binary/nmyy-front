...
<script>
import AddDialog from './user-add-dialog.vue'
import RoleModifyDialog from './user-modify-dialog'
import RecordTable from './user-table.vue'
import { userList, deleteUser, roleList } from '@/api/user'
...
export default {
    ...
    methods: {
        initOptions () {
            roleList().then(res => {
                if (res.code === "00000" && res.data) {
                    this.nodeOptions = res.data.filter(item => item.status != 2);
                }
            });
        },
        handleDelete (row) {
        ...
        initTable () {
            userList({ name: this.keyword, page: this.currentPage, pageSize: this.pageSize }).then((res) => {
                if (res.code === "00000" && res.data) {
