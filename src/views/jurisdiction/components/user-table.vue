<template>
    <div class="record-table-con">
        <i-table :tableData="tableData">
            ...
            <el-table-column prop="accountName" :show-overflow-tooltip="true" min-width="12%" label="用户名">
            </el-table-column>
            <el-table-column prop="mobile" :show-overflow-tooltip="true" min-width="12%" label="手机号">
            </el-table-column>
            <el-table-column prop="email" :show-overflow-tooltip="true" min-width="12%" label="邮箱">
            </el-table-column>
            <el-table-column prop="slug" min-width="12%" label="角色">
                <template v-slot="scoped">
                    <template v-if="scoped.row.roles && Object.keys(scoped.row.roles).length">
                        <span class="jur-tag" v-for="(item, index) in scoped.row.roles" :key="index">{{
                            item.roleName
                        }}</span>
                    </template>
                    <span v-else>-</span>
